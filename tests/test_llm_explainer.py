...
def test_generate_explanation_ollama(mock_post, dummy_context):
    ...
    mock_response.json.return_value = {"message": {"content": "This is a mocked explanation."}}
...
def test_ask_question_ollama(mock_post, dummy_context):
    ...
    mock_response = MagicMock()
    mock_response.json.return_value = {"message": {"content": "It means age is important."}}
    mock_response.raise_for_status.return_value = None
    mock_post.return_value = mock_response
...
def test_what_if_ollama(mock_post, dummy_context):
    ...
    mock_response = MagicMock()
    mock_response.json.return_value = {"message": {"content": "Prediction changes because age increased."}}
    mock_response.raise_for_status.return_value = None
    mock_post.return_value = mock_response
