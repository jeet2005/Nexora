...
def init_dvc(repo_path: str = "."):
    ...
    if not os.path.exists(dvc_dir):
        try:
            subprocess.run(
                ["dvc", "init"], cwd=repo_path, check=True, capture_output=True
            )
...
def snapshot_data(
    df: pd.DataFrame, dataset_name: str, version_tag: str, repo_path: str = "."
) -> str:
    ...
    try:
        # Add to DVC
        subprocess.run(
            ["dvc", "add", file_path], cwd=repo_path, check=True, capture_output=True
        )

        # Git commit the .dvc file
        dvc_file = f"{file_path}.dvc"
        subprocess.run(
            ["git", "add", dvc_file, os.path.join(repo_path, ".dvc/config")],
            cwd=repo_path,
            check=False,
            capture_output=True,
        )
        subprocess.run(
            ["git", "commit", "-m", f"Track data {dataset_name} version {version_tag}"],
            cwd=repo_path,
            check=False,
            capture_output=True,
        )
        ...
        raise RuntimeError(
            f"Failed to track data with DVC: {e.stderr.decode() if e.stderr else str(e)}"
        ) from e
