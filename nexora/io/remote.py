...
def load_from_sql(query: str, connection_string: str | None = None) -> pd.DataFrame:
    ...
    try:
        ...
        raise ImportError("DuckDB is required for from_sql. It should be installed with nexora.") from e
...
def load_from_mongodb(uri: str, collection: str) -> pd.DataFrame:
    ...
    try:
        ...
        raise ImportError("pymongo is required for from_mongodb. Run `pip install nexora[mongo]`") from e
...
def load_from_s3(bucket: str, key: str) -> pd.DataFrame:
    ...
    try:
        ...
        raise ImportError("boto3 is required for from_s3. Run `pip install nexora[cloud]`") from e
...
def load_from_sklearn(dataset_name: str) -> pd.DataFrame:
    ...
    try:
        ...
        raise ImportError("scikit-learn is required. It should be installed with nexora.") from e
    ...
    loaders = {
        ...
        "california_housing": getattr(sklearn.datasets, "fetch_california_housing", None),
    }
    
    name = dataset_name.lower().replace("-", "_")
    if name not in loaders or loaders[name] is None:
        raise ValueError(f"Sklearn dataset '{dataset_name}' not supported. Options: {list(loaders.keys())}")
