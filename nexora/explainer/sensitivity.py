...
def sensitivity(
    model, X: pd.DataFrame, feature: str, stdev_multiplier: float = 1.0
) -> dict[str, float]:
    ...
    if not pd.api.types.is_numeric_dtype(X[feature]):
        raise ValueError(
            f"Sensitivity analysis currently only supports numeric features. '{feature}' is not numeric."
        )
    ...
    mean_pct_change = (
        np.mean(
            np.abs((perturbed_preds - baseline_preds) / (np.abs(baseline_preds) + 1e-9))
        )
        * 100
    )
