...
def plot_roc_curve(
    y_true: pd.Series | np.ndarray,
    y_prob: pd.DataFrame | np.ndarray,
    labels: list[str] | None = None,
) -> plt.Figure:
    ...
    if len(y_prob.shape) == 1 or y_prob.shape[1] <= 2:
        ...
        for i, cls in enumerate(classes):
            ...
            RocCurveDisplay.from_predictions(
                y_true_bin[:, i], y_prob[:, i], name=name, ax=ax
            )
...
def plot_pr_curve(
    ...
    if len(y_prob.shape) == 1 or y_prob.shape[1] <= 2:
        ...
        PrecisionRecallDisplay.from_predictions(
            y_true, prob, name="Positive Class", ax=ax
        )
    else:
        from sklearn.preprocessing import label_binarize
...
def plot_learning_curve(
    estimator: Any, X: pd.DataFrame | np.ndarray, y: pd.Series | np.ndarray, cv: int = 5
) -> plt.Figure:
...
def plot_calibration_curve(
    y_true: pd.Series | np.ndarray, y_prob: pd.DataFrame | np.ndarray
) -> plt.Figure:
    ...
    if len(y_prob.shape) == 1 or y_prob.shape[1] <= 2:
        ...
        CalibrationDisplay.from_predictions(
            y_true, prob, n_bins=10, ax=ax, name="Model"
        )
    else:
        ...
        CalibrationDisplay.from_predictions(
            correct, max_prob, n_bins=10, ax=ax, name="Top-1 Confidence"
        )
