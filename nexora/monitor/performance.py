...
def error_analysis(y_true: pd.Series | np.ndarray, y_pred: pd.Series | np.ndarray, features: pd.DataFrame, task_type: str = "regression") -> pd.DataFrame:
    ...
    for col in features.columns:
        if pd.api.types.is_numeric_dtype(features[col]) and features[col].nunique() > 10:
            ...
            try:
                ...
                grouped = df_eval.groupby(binned)["_nexora_error"].agg(["mean", "count"])
            except Exception:
                continue
        else:
