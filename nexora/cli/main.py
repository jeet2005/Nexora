...
def info() -> None:
    ...
    info_table = Table(
        title="Nexora Information", show_header=True, header_style="bold cyan"
    )
...
@cli.command()
@click.argument(
    "data_csv", type=click.Path(exists=True, dir_okay=False, path_type=Path)
)
...
@click.option(
    ...
    show_default=True,
    type=int,
    help="Requested cross-validation folds.",
)
@click.option(
    "--timeout",
    "timeout_sec",
    default=None,
    type=int,
    help="Optional per-model timeout in seconds.",
)
@click.option(
    "--seed",
    "random_state",
...
@click.option(
    ...
    default=True,
    show_default=True,
    help="Enable early stopping when supported.",
)
def train(
    data_csv: Path,
    target: str,
...
def profile(data_csv: Path, export: Path | None) -> None:
    ...
    health_table = Table(
        title="Dataset Health Profile", show_header=True, header_style="bold cyan"
    )
    ...
    if export:
        ...
        export_path.write_text(
            f"<h1>Profile: {prof.source_name}</h1>", encoding="utf-8"
        )
...
def quick(data_csv: Path, target: str) -> None:
    ...
    console.print(
        f"[green]✓ Best:[/green] {report.best_model} | [green]Score:[/green] {report.best_score:.4f}\n"
    )
...
@cli.command()
@click.argument(
    "model_nx", type=click.Path(exists=True, dir_okay=False, path_type=Path)
)
@click.argument(
    "new_data", type=click.Path(exists=True, dir_okay=False, path_type=Path)
)
@click.option(
    "--output",
    type=click.Path(dir_okay=False, path_type=Path),
...
@click.option(
    "--model", default=None, help="Specific model to explain (default: best model)."
)
    ...
    try:
        ...
        if isinstance(explanation, dict) and "feature_importance" in explanation:
            for i, (feat, score) in enumerate(
                list(explanation["feature_importance"].items())[:top_features], 1
            ):
...
@click.option(
    "--sample-size", default=100, type=int, help="Number of samples to analyze."
)
def whatif(model_nx: Path, new_data: Path, sample_size: int) -> None:
    ...
    try:
        ...
        for idx, row in predictions.head(10).iterrows():
            pred_col = (
                [c for c in predictions.columns if "predicted" in c.lower()][0]
                if any("predicted" in c.lower() for c in predictions.columns)
                else predictions.columns[0]
            )
            conf = row.get("confidence", "N/A")
            what_if_table.add_row(
                f"Scenario {idx + 1}", str(row[pred_col])[:20], str(conf)[:10]
            )
...
def cluster(data_csv: Path, n_clusters: int) -> None:
    ...
    try:
        ...
        cluster_table = Table(
            title="Cluster Analysis", show_header=True, header_style="bold cyan"
        )
...
@click.option(
    "--freq", type=click.Choice(["D", "W", "M"]), default="M", help="Frequency."
)
def forecast(
    data_csv: Path, date_col: str, target_col: str, periods: int, freq: str
) -> None:
    ...
    try:
        ...
        forecast_table = Table(
            title="Forecast Results", show_header=True, header_style="bold cyan"
        )
        ...
        for row in result.get("forecast", [])[:24]:
            forecast_table.add_row(
                str(row.get("date", "?")), f"{row.get('prediction', 0):.4f}"
            )
...
def compare(model_nx: Path, limit: int) -> None:
    ...
    compare_table = Table(
        title="Model Comparison", show_header=True, header_style="bold cyan"
    )
...
def models(category: str | None, task: str | None) -> None:
    ...
    try:
        ...
        console.print(
            f"[bold cyan]Available models in Nexora ({len(all_models)}):[/bold cyan]\n"
        )
